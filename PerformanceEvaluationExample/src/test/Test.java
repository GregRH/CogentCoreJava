...
public class Test {
	...
	public static void main(String[] args) {
		...
		count1 = painterList.stream().filter(x-> (x.getName().equals("John") ||x.getPaintingStyle().equals("charcole"))).filter(x-> x.getTheme().equals("Invention")).filter(x-> x.getName().startsWith("J")).count();
		long t2 = System.currentTimeMillis();
		System.out.println("Count = " + count1+ " sequentialStream Takes " + (t2-t1) + " ms\n");
		long t3 = System.currentTimeMillis(), count2;
		count2 = painterList.parallelStream().filter(x-> (x.getName().equals("John") || x.getPaintingStyle().equals("charcole"))).filter(x-> x.getTheme().equals("Invention")).filter(x-> x.getName().startsWith("J")).count();
